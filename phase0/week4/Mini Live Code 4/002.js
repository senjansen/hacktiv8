...
function printBranch (branches) {
  // Declare the months in an array
  let month = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC']
  // Create a string to accomodate the end result
  let result = ''
  // Do loops for every month display
  for (let i = 0; i < month.length; i++) {
    // Add month first to the row
    result = `${result} | ${month[i]} |`
    // Do another loop to get the income and outcome of every branch
